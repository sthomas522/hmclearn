...
We load drug sleepstudy data (Agresti 2015, Belenky et. al. 2003) and create the design matrices $X$ and $Z$ and dependent vector $y$.  For this model, the random effects design matrix $Z$ is specified for a random intercept model.  
...
```{r, echo=TRUE, fig.width=6, fig.height=4}
beta.true <- fixef(fm1)
xi.true <- 1/2*log(VarCorr(fm1)$Subject[1])
sigeps.true <- log(sigma(fm1))
theta.true <- c(beta.true, sigeps.true, xi.true)
diagplots(f_hmc, burnin=trunc(N*.3), 
          actual.mu=theta.true, cols=c(1:2, 21:22))
```
