...
test1 <- g_glmm_poisson_posterior(thetaInit, y=y, X=X, Z=Z, m=10, q=1, nuxi=4, Axi=1)
...
STAN manual section 9.2 The QR Reparameterization
...
##########
# block diagonal
Zi.lst <- split(rep(diagval, nrow(Gdat)), Gdat$Site)
Zi.lst <- lapply(Zi.lst, as.matrix)
Z2 <- bdiag(Zi.lst)
Z2 <- as.matrix(Z2)
...
Small data set.  We use a more informative prior on the random effects variance.  Per STAN's recommendation, we use a half-t prior with 4 degrees of freedom and scale parameter of 1.  
...
p <- ncol(X)
res2$thetaDF[, 1:p] <- t(apply(res2$thetaDF[, 1:p], 1, calc_beta, Rstarinv=Rstar_inv))
...
```{r,echo=TRUE}
plot(res, burnin=round(0.3*N), actual.mu=truevals)
