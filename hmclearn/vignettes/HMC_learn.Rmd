...
Per Betancourt, Girolami (2013), we re-parameterize $u$ using a standard normal parameterization we define as $\tau = (\tau_1, ..., \tau_M)$.  Here, $u$ is a deterministic function of $D$ and $\tau$.  The intent of our parameterization is to allow $D$ and $\tau$ to be largely independent in the MCMC sampling.  
...
We assign half-t priors for each of the parameters $\lambda_1, ..., \lambda_M$ (Gelman 2006) with hyperparameters $\nu$ and $A$.  
...
As a comparison, we fit the model using the Frequentist package \textit{lme4}
