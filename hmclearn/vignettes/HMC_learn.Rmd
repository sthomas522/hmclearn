...
The default number of simulations is set to 10,000.  However, the user may elect to start with a smaller number during tuning.  Defaults are provided for the stepsize $\epsilon$ and leapfrog steps $L$.  These likely will need to be adjusted depending on the particular application.  

The output of \textit{hmc} is a single list containing the model results.  The acceptance rate can be calculated from the \textit{hmc} function from the number of accepted proposals.  
