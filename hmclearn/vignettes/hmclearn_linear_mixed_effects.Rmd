---
title: "hmclearn_linear_mixed_effects"
author:  "Samuel Thomas"
date: "``r Sys.Date()`"
output: rmarkdown::html_vignette
vignette: >
  %\VignetteIndexEntry{hmclearn_linear_mixed_effects}
...
# not independent random effects
...
# library(nlme)
# fm2 <- lme(Reaction ~ Days, random = ~ Days | Subject, data =sleepstudy)
# summary(fm2)
```
...
```{r, echo=TRUE}
require(Matrix)
# G from frequentist fit
Gtrue <- matrix(as.numeric(VarCorr(fm1)$Subject[1:4]), ncol=2)
Gtrue
...
colnames(X2) <- paste(colnames(X), "qr", sep="_")
...
theta.init <- c(0,0, # beta 
               rep(0, 36), # tau
               1, # gamma (log sig2eps)
               c(1, 1, 0)) # xi and a (log G diagonal and a off-diagonal)

vnames <- c(paste0("beta", 0:1), 
           paste0(rep(c("tau_int", "tau_slope"), m),rep(c(1:18), each=2) ),
           "sigeps", "xi_1", "xi_2", "a")
...
Estimated precision matrix 
    ...
    beta0     beta1    u_int1  u_slope1    u_int2  u_slope2    u_int3  u_slope3    u_int4  u_slope4    u_int5  u_slope5 
...
```{r, echo=TRUE, eval=T}
xi_1_true <- 1/2*log(VarCorr(fm1)$Subject[1])
a_true <- VarCorr(fm1)$Subject[2]/VarCorr(fm1)$Subject[1]
xi_2_true <- 1/2*log(VarCorr(fm1)$Subject[4] - a_true^2 * exp(2*xi_1_true))
```
